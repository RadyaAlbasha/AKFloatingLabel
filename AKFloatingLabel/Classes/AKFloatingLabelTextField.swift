...
@IBDesignable open class AKFloatingLabelTextField: UITextField, UITextFieldDelegate {
    ...
    public var floatingLabelFont: UIFont? {
        didSet {
            floatingLabel.font = (self.floatingLabelFont != nil) ? self.floatingLabelFont : self.defaultFloatingLabelFont()
    ...
    @IBInspectable public var floatingLabelReductionRatio: CGFloat = 70 {
        didSet {
            floatingLabelFont = self.defaultFloatingLabelFont()
    ...
    /**
     * Indicates whether or not to drop the baseline when entering text. Setting to true (not the default) means the standard greyed-out placeholder will be aligned with the entered text
    ...
    func updateDefaultFloatingLabelFont() {
        let derivedFont = self.defaultFloatingLabelFont()
    ...
    func setLabelErrorOriginForTextAlignment() {
        floatingLabelError.frame = CGRect(x: 0, y: self.frame.size.height + 2.0, width: 300, height: 20.0)
    }

    func setBorder() {
        if hasBottomBorder {
            bottomBorder.borderColor = validTextFieldColor.cgColor
            bottomBorder.frame = CGRect(x: 0, y: self.frame.size.height, width: self.frame.size.width, height: 1)
    ...
    override open func textRect(forBounds bounds: CGRect) -> CGRect {
        ...
        if !textIsEmpty || keepBaseline {
            rect = self.insetRect(forBounds: rect)
        }
    ...
    override open func clearButtonRect(forBounds bounds: CGRect) -> CGRect {
        ...
        let floatingLabelIsEmpty = (self.floatingLabel.text?.isEmpty ?? true)
